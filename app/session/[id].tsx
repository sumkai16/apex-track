...
export default function SessionScreen() {
  ...
  async function saveSetToDb(
    peId: string,
    setIndex: number,
    updatedSets: Record<string, LoggedSet[]>,
  ) {
  ...
  async function saveAllSets() {
    ...
    (historicalSets ?? []).forEach((s) => {
      if (
        !historicalMax[s.exercise_id] ||
        s.weight_used > historicalMax[s.exercise_id]
      ) {
    ...
    allSets.forEach((set) => {
      if (
        !sessionMax[set.exercise_id] ||
        set.weight_used > sessionMax[set.exercise_id]
      ) {
        ...
        <View style={styles.exCard}>
          ...
          {currentSets.map((set, i) => {
            ...
            return (
              ...
              >
                <Text
                  style={[styles.setNum, isCurrentSet && styles.setNumActive]}
                >
                  {set.set_number}
                </Text>
                <TextInput
                  style={[
                    styles.setInput,
                    isCurrentSet && styles.setInputActive,
                  ]}
                  value={
                    weightInputs[`${currentExercise.id}-${i}`] ??
                    (set.weight_used > 0
                      ? String(toDisplay(set.weight_used))
                      : "")
                  }
                  onChangeText={(v) => {
                ...
                <TextInput
                  ...
                  onChangeText={(v) =>
                    updateSet(currentExercise.id, i, "reps_done", v)
                  }
                  keyboardType="numeric"
                  placeholder="0"
                  placeholderTextColor="#444"
                ...
                >
                  <Text
                    style={[styles.checkText, set.done && styles.checkTextDone]}
                  >
                    ✓
                  </Text>
                </TouchableOpacity>
