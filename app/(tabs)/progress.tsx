...
export default function ProgressScreen() {
    ...
    async function fetchSessions(userId: string) {
        ...
        const dayIds = [...new Set(userSessions.map(s => s.program_day_id).filter(Boolean))]
        ...
        const dayMap = new Map<string, { day_name: string; program_name: string }>()
        days?.forEach((d: any) => {
            dayMap.set(d.id, {
                day_name: d.name,
        ...
        const result: SessionSummary[] = userSessions.map(s => ({
            ...
            program_name: dayMap.get(s.program_day_id)?.program_name ?? 'Unknown Program',
        }))
                    ...
                    <>
                        {sessions.length === 0 ? (
                            <View style={styles.empty}>
                                ...
                                <Text style={styles.emptyText}>Complete a session to see your history here.</Text>
                            </View>
                        ) : (
                            sessions.map(s => (
                                <TouchableOpacity
                                    key={s.session_id}
                                    style={styles.card}
                                    onPress={() => router.push({ pathname: '/session-detail/[id]', params: { id: s.session_id } })}
                                    activeOpacity={0.8}
                                >
                                    <View style={styles.cardTop}>
...
const styles = StyleSheet.create({
    ...
    arrow: { color: '#800000', fontSize: 20, position: 'absolute', right: 16, top: '70%' },
